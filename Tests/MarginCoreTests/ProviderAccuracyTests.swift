...
final class ProviderAccuracyTests: XCTestCase {
    ...
    func testClaudeProviderRendersExactlyTheSource() async throws {
        ...
        let loaded = await ClaudeProvider(configURL: url, live: false).load(forceLive: false)
    ...
    func testClaudeProviderReturnsNilWhenFileMissing() async {
        ...
        let loaded = await ClaudeProvider(configURL: missing, live: false).load(forceLive: false)
        XCTAssertNil(loaded)
    }
    ...
    func testCodexProviderNormalWindowsAndPlan() async throws {
        ...
        let loaded = await CodexProvider(root: root, live: false).load(forceLive: false)
        let snapshot = try XCTUnwrap(loaded)
